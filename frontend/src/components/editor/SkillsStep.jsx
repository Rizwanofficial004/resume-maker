...
export default function SkillsStep({ items: rawItems, setItems, jobTitle }) {
  ...
  const fetchSuggestions = useCallback(async (regenerate = false) => {
    ...
    try {
      if (regenerate && (user?.aiCredits ?? 0) > 0) {
  ...
  }, [category, items, jobTitle, user, updateUser]);

  const addSkill = (name) => {
    if (!name.trim()) return;
    if (items.some(s => s.name.toLowerCase() === name.toLowerCase())) return;
    const newOne = { ...newSkill(), name: name.trim() };
  ...
  const updateSkillLevel = (id, level) => {
    setItems(items.map(s => s.id === id ? { ...s, level } : s));
  };
    ...
    <MuiProvider>
      <div>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                    ...
                    {showLevels && (
                      <Box sx={{ display: 'flex', gap: '2px', ml: 1 }}>
                        {[1,2,3,4,5].map(l => (
                          <Box key={l} onClick={() => updateSkillLevel(skill.id, l)} sx={{
                            width: 12, height: 5, borderRadius: 1, cursor: 'pointer',
                            bgcolor: l <= skill.level ? 'primary.main' : 'grey.300',
                            '&:hover': { bgcolor: l <= skill.level ? 'primary.dark' : 'grey.400' },
                          }} />
                        ))}
                      </Box>
                    )}
        ...
        <Box sx={{ display: 'flex', gap: 1.5, mt: 3 }}>
          ...
          />
          <button onClick={addCustomSkill} disabled={!inputValue.trim()}
        ...
        <div className="mb-3 flex items-center justify-between">
          ...
          {suggestions.length > 0 && regenerateCount < maxRegenerations && (
            <button onClick={() => fetchSuggestions(true)} disabled={loadingSuggestions}
              ...
              Regenerate ({regenerateCount + 1}/{maxRegenerations})
            </button>
          )}
        </div>
        {suggestions.length === 0 && !loadingSuggestions ? (
          <button onClick={() => fetchSuggestions(false)}
            className="w-full rounded-xl border-2 border-dashed border-slate-200 py-6 text-sm font-medium text-slate-500 hover:border-blue-300 hover:text-blue-600 transition">
            <Sparkles size={16} className="mr-1.5 inline text-amber-500" />
            Get AI-suggested skills for &quot;{jobTitle || 'your role'}&quot;
          </button>
        ) : (
